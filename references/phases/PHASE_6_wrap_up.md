...
2. Master Traceability Matrix (Register → audience coverage GREEN/YELLOW/RED).
   ...
   `references/protocols/ARTIFACT_STATUS.md` Counting rule).
...
9. **STATUS Distribution** — counts per status across all artifacts, AND
    **Root Cause Aggregation** (per `references/protocols/ROOT_CAUSE_GROUPING.md`):
...
15. **Fixture Report** (per `references/protocols/TEST_FIXTURES.md`): fixture
...
`references/protocols/QUALITY_GATES.md`): the user (in team settings,
