...
`08-protocols/ARTIFACT_STATUS.md` and never mix on one row.
...
4. **Layer** — same architectural tier, using the canonical layer tags from `99-reference/ID_SCHEMES.md` (e.g., CONTROLLER / SERVICE / MIDDLEWARE / FRONTEND_UI / REPOSITORY / SCHEDULER)
...
vocabularies, per `99-reference/ID_SCHEMES.md`; dedup matches on the
...
`01-phases/PHASE_4_agent_swarm.md` — the Orchestrator emits:
